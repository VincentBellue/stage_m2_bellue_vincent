...
output.write("Ligne" + "\t" + "Compositionnel" + "\t" "Base" + "\t" + "Vecteur base" + "\t" + "Collocatif" + "\t" + "Vecteur collocatif" + "\t" + "Phrase" + "\n")#entête des colonnes du fichier en output
...
for ligne in tableau:
	...
	base = decoupage[0] #base = 1ère colonne
	collocatif = decoupage[1] #collocatif = 2ème colonne
	trait_compositionnel = decoupage[2]#trait compositionnel = 3ème colonne
	phrase = decoupage[3]#phrase entière = 4ème colonne
	...
	liste_info.append([numero_ligne_phrase,base,collocatif,trait_compositionnel,phrase])#ajout du contenu des colonnes dans la liste vide liste_info
	cpt+=1
...
for ln in liste_info:#boucle qui parcourt la liste créée précédemment avec toutes les informations de chaque ligne
	...
	tok_base=ln[1]#base
	tok_colloc=ln[2]#collocatif
	trait_compo=ln[3]#compositionnalité oui/non
	phrase_entiere=ln[4]#phrase entière
	...
	output.write(str(num_phrase)+"\t"+trait_compo+"\t"+ tok_base+"\t"+ " ".join(map(str, vec_base))+"\t"+tok_colloc+"\t"+ " ".join(map(str,vec_colloc))+"\t"+phrase_entiere+"\n")
